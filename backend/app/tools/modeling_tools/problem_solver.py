...
Estados posibles: optimal, infeasible, unbounded, error."""
    ...
    # Parsing is shared with SimplexSolverTool via ``_lp_parsing``. These thin
    # wrappers preserve the existing method API and scipy sign convention.
    @staticmethod
    def _parse_variables(
        variables: list[dict[str, Any]],
    ) -> tuple[list[str], list[str], list[tuple[float | None, float | None]]]:
        """Parse variable definitions. Returns (names, types, bounds)."""
    ...
    def _parse_objective(
        ...
        """
        Parse objective function, negating for maximization since scipy minimizes.

        Returns:
            Tuple of (coefficient_vector, is_maximization)
        """
        coefficients, is_maximization = parse_objective_coefficients(
            objective, var_names
        )
        if is_maximization:
    ...
    def _parse_constraints(
        ...
        """Parse constraints. Returns (A_ub, b_ub, A_eq, b_eq, constraint_names)."""
        return parse_constraints(constraints, var_names)
    ...
    def _parse_linear_expression(
        ...
        return parse_linear_expression(expression, var_names)

    @staticmethod
    def _parse_term(term: str, var_names: list[str]) -> tuple[float, str | None]:
        """Parse a single term like "3*x1", "-x2", "5", "x1"."""
        return parse_term(term, var_names)

    @staticmethod
    def _parse_constraint_expression(
        expression: str, var_names: list[str]
    ) -> tuple[list[float], float, str]:
        """Parse a constraint expression. Returns (lhs, rhs, sense)."""
        return parse_constraint_expression(expression, var_names)
