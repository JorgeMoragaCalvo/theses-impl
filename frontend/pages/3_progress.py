...
if success:
    ...
    with col3:
        created_date = datetime.fromisoformat(
            student_data["created_at"].replace("Z", "+00:00")
        )
        ...
        for conv in conversations[:5]:
            with st.expander(
                ...
                if st.button("View Details", key=f"view_{conv['id']}"):
                    ...
                    if detail_success:
                        st.write(
                            f"**Messages:** {len(detail_data.get('messages', []))}"
                        )
    elif conv_success:
        st.info("No conversations yet.")
    else:
        st.error(
            f"Failed to load conversations: {conversations.get('error', 'Unknown error')}"
        )
    ...
    if progress_data is not None:
        ...
        with col1:
            st.metric("Total Mensajes", progress_data.get("total_messages", 0))

        with col2:
            st.metric(
                "Problemas de práctica", progress_data.get("total_assessments", 0)
            )

        with col3:
            avg_score = progress_data.get("average_score")
            if avg_score is not None:
                st.metric("Puntuación media", f"{avg_score:.1f}")
            else:
                st.metric("Average Score", "N/A")

        with col4:
            st.metric(
                "Total Conversaciones", progress_data.get("total_conversations", 0)
            )
    else:
        st.error("Unable to load learning statistics.")
else:
    error_msg = student_data.get(
        "error", student_data.get("detail", "Failed to load student data")
    )
